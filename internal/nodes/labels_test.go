...
func TestLabelForTree_PrimaryPoolSelection(t *testing.T) {
	...
	// node1 matches both MCPs; after processing both trees vertically
	// workerB should win (custom > worker in MCO priority)
	...
	allNodes := []corev1.Node{*node1, *node2}

	treeWorker := nodegroupv1.Tree{
		NodeGroup:          &nropv1.NodeGroup{},
		MachineConfigPools: []*mcov1.MachineConfigPool{worker},
	}
	treeWorkerB := nodegroupv1.Tree{
		NodeGroup:          &nropv1.NodeGroup{},
		MachineConfigPools: []*mcov1.MachineConfigPool{workerB},
	}

	if err := LabelForTree(ctx, cli, platform.OpenShift, treeWorker, allPools, allNodes); err != nil {
		t.Fatalf("unexpected error labeling worker tree: %v", err)
	}
	if err := LabelForTree(ctx, cli, platform.OpenShift, treeWorkerB, allPools, allNodes); err != nil {
		t.Fatalf("unexpected error labeling workerB tree: %v", err)
	}
...
func TestLabelForTree_StaleLabelCorrected(t *testing.T) {
	worker := newTestMCPWithSelector("worker", map[string]string{"node-role.kubernetes.io/worker": ""})

	// node matches the worker MCP but has a stale label from a previous config
	...
	allPools := []mcov1.MachineConfigPool{*worker}
	...
	val, ok := getLabel(ctx, cli, "node1")
	if !ok || val != "worker" {
		t.Fatalf("expected stale label on node1 to be corrected to worker, got %q (exists=%v)", val, ok)
	}
}

func TestLabelForTree_UnmanagedPoolNotLabeled(t *testing.T) {
	worker := newTestMCPWithSelector("worker", map[string]string{"node-role.kubernetes.io/worker": ""})
	infra := newTestMCPWithSelector("infra", map[string]string{"node-role.kubernetes.io/infra": ""})

	// node matches infra MCP, but infra is not in the managed tree
	node := newTestNode("node1", map[string]string{
	...
	allPools := []mcov1.MachineConfigPool{*worker, *infra}
	allNodes := []corev1.Node{*node}
	tree := nodegroupv1.Tree{
		NodeGroup:          &nropv1.NodeGroup{},
		MachineConfigPools: []*mcov1.MachineConfigPool{worker},
...
func TestLabelForTree_HyperShiftNoOp(t *testing.T) {
	...
	if err := LabelForTree(ctx, cli, platform.HyperShift, tree, nil, nil); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestLabelForTree_Idempotent(t *testing.T) {
	worker := newTestMCPWithSelector("worker", map[string]string{"node-role.kubernetes.io/worker": ""})
	node := newTestNode("node1", map[string]string{
		"node-role.kubernetes.io/worker": "",
	})
